  ...
  <div class="app-container product-wrapper">
    <div class="form-wrapper">
      <el-form ref="ruleForm" :model="ruleForm" :rules="rules" label-width="100px">
        <el-form-item label="软件类型:" prop="name">
          <el-input v-model="ruleForm.name" placeholder="软件类型代码值" class="width-50p" />
        </el-form-item>
...
<script>
import { saveOrUpdate } from '@/api/software-type'
        ...
        if (valid) {
          ...
          saveOrUpdate(this.ruleForm, method).then(response => {
            if (response.status === 201 || response.status === 200) {
              this.$message({
                message: `${this.buttonText}软件类型成功！`,
                type: 'success'
              })
